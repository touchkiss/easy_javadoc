# Spec: record-batch-generation

## Purpose

Define the batch generation behaviour for Java record components: the UI checkbox in the "Generate All" dialog, persistence of that setting, and the runtime iteration logic that calls `getRecordComponents()` to process record components separately from fields.
