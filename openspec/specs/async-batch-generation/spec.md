# Spec: Async Batch Javadoc Generation

## Purpose

批量注释生成（`GenerateAllJavadocAction`、`GenerateClassMembersJavadocAction`、`ProtobufMembersHandler`）的翻译/注释生成阶段在后台线程执行，避免阻塞 EDT，并通过单次 `WriteCommandAction` 合并写入，确保撤销体验一致。
